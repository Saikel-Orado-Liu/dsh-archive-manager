window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* Client contribution for the host `workspaceRegistry` Remote methods
		* added by dsh-archive-manager-workspace. Mounted through
		* `ctx.remote.$mount`; calls ride the typert gateway SRC path and keep
		...
		*/
		...
		//#endregion
		//#region dsh 0.1.5 uiWorkspace compatibility
		/** Structured directory failure exposed to current directory UI consumers. */
		...
		/** Navigation service required by the current conversation/sidebar bundles. */
		var UiWorkspaceService = class extends _deepseek_ai_cordis.Service {
			...
			constructor(ctx, directoryPicker, workspaces, sessions) {
				...
				ctx.effect(() => this.watchNavigation(), "archive-manager: Workspace navigation policy");
			}
			...
			startSession(workspaceId) {
				...
				this.connectWorkspace(target).then((sessionId) => this.sessions.open(sessionId), (reason) => console.warn("new session failed:", reason));
			}
			...
			watchNavigation() {
				...
				const reconcile = () => {
					...
					this.connectWorkspace(target).then((sessionId) => {
						if (!disposed && this.sessions.list.getSnapshot().current === void 0) this.sessions.open(sessionId);
						initial = "done";
					}, (reason) => {
						if (!disposed) {
							initial = "waiting";
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.HoverCard, {
				anchor: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsx)("span", {
							className: clsx(Rows_module_css_default.title, archived && ARCHIVED_CLASSES.title),
							children: title
						}),
		...
		*/
		...
		/** Grouping, ordering, and the show-archived toggle; own open state so it resets with the wide chrome. */
		function ViewOptionsMenu({ groupBy, orderBy, showArchived, onGroupPick, onOrderPick, onShowArchivedPick, t }) {
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
				...
				selectedIds: [...(showArchived === true ? ["show-archived"] : []), groupBy, orderBy],
				onSelect: (id) => {
					if (id === "workspace" || id === "flat") onGroupPick(id);
					else if (id === "manual" || id === "updated") onOrderPick(id);
					else if (id === "show-archived") onShowArchivedPick(!(showArchived === true));
					setOpen(false);
				},
				align: "end",
				dense: true,
						...
						children: [
							...
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [wide && (0, react_jsx_runtime.jsx)(ViewOptionsMenu, {
									...
									onShowArchivedPick: (value) => {
										actions.setShowArchived(value);
									},
									t
		...
		*/
		async function apply(ctx) {
			...
			const uiWorkspaceService = new UiWorkspaceService(ctx, ctx.remote.directoryPicker, workspaces, sessions);
			let disposeRemote = () => {};
			if (remote !== void 0) disposeRemote = await remote.$mount(ARCHIVE_MANAGER_REMOTE);
			applyWorkspaceBrowser(ctx, uiWorkspaceService);
			return async () => {
				await disposeRemote();
			};
		}
		...
		* @param ctx - client root context.
		* @param uiWorkspaceService - DSH 0.1.5 workspace navigation service.
		*/
		function applyWorkspaceBrowser(ctx, uiWorkspaceService) {
			ctx.effect(() => ctx.locale.register(NS, {
				zh,
				en
			}), "dsh-archive-manager: dictionaries");
			// DSH 0.1.5 resolves the sidebar workspace hook from the root slot
			// ledger. The older fork relied on the pre-0.1.5 owner to provide it.
			...
			const browserInjected = () => ({
				...
				unarchiveSession: async (sessionId) => {
					// Explicit service read: the namespace is mounted by this
					// plugin's own apply (before any UI action can run), and the
					// proxy property path would require declaring
					// "remote.workspaceRegistry" in inject — which cannot be
					// declared while the same fiber performs the $mount.
